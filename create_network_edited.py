...
# Written by b8vrede, forked from: https://github.com/b8vrede/new_interface_wetten.nl

# Two files that are needed for this script:
# https://github.com/b8vrede/new_interface_wetten.nl/blob/master/caselaw.edges
# https://github.com/b8vrede/new_interface_wetten.nl/blob/master/laws.edges
#
# EDITED FOR CASE LAWS (see the bottom of this pace)
# Edited references network file, to give suggestions from case law to other case law
...
# DEZE om lokale netwerk te krijgen        
...
# Disabled because only the case law suggestions are needed
if False:
...
# CASELAW Edit: open this file locally
fh=open("caselaw.edges", 'rb')
CaseG=nx.read_weighted_edgelist(fh)
fh.close()

# Caselaw edit:
current_ecli = 'ECLI:NL:RBSGR:2009:BK7528'

current_node = '<http://rechtspraak.nl/ecli?id=' + current_ecli + '>'

endpoints = True

#CaseG = nx.read_weighted_edgelist("/var/www/wettenbart/caselaw.edges") # , create_using=nx.DiGraph() <-- Used for creating directional graphs

#local_graph = ego_graph(CaseG, current_node, radius = 2, center = True, undirected = True, distance='weight')
local_graph = ego_graph(CaseG, current_node, radius = 1.3, center = True, undirected = True, distance='weight')
# print 'debug 1'
